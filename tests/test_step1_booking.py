...
def test_ship_sticks_booking_step_1_happy_path(page: Page):
   ...
   # Retrieve displayed addresses
    logger.info("Retrieving displayed addresses")
    ...
    print(f"Origin assertion passed: '{actual_origin}' is in '{DataManager.origin}'")  # Console log for origin assertion
    assert actual_destination in DataManager.destination, f"Expected '{DataManager.destination}' to contain '{actual_destination}'"
    print(f"Destination assertion passed: '{actual_destination}' is in '{DataManager.destination}'")  # Console log for destination assertion
    ...
    print(f"Shipment Cities from order summary assertion passed : {shipment_cities}")  # Console log for shipment cities
    ...
    print(f"Payment summary items assertion passed: {payment_items}")  # Console log for payment summary items
    ...
    print(f"Total price assertion passed: {total_price}")  # Console log for total price

    ship_page.complete_step1_booking()

     # Verify check-circle icon is visible
    logger.info("Verifying checked icon visibility")
    expect(ship_page.checked_icon).to_be_visible()
    print("Step 1 booking completed green Checked icon visibility assertion passed")  # Console log for checked icon visibility
    print("Step 1 booking flow completed successfully")  # Console log for test completion
